		...
		Commands: []*cli.Command{
			...
			},
			{
				...
				Action: func(_ context.Context, cmd *cli.Command) error {
					...
					raw, err := req(c, orgID, "GET", "/organizations/"+cmd.Args().First(), nil)
					if err != nil {
						return err
					}
					return output.JSONRaw(raw)
			...
			},
			{
				...
				Action: func(_ context.Context, cmd *cli.Command) error {
					...
					raw, err := c.Request("POST", c.InventoryBase+"/organizations", &zohttp.RequestOpts{JSON: body})
					if err != nil {
						return err
					}
					return output.JSONRaw(raw)
			...
			},
			{
				...
				Action: func(_ context.Context, cmd *cli.Command) error {
					...
					var body any
					json.Unmarshal([]byte(cmd.String("json")), &body)
					raw, err := req(c, orgID, "PUT", "/organizations/"+cmd.Args().First(), &zohttp.RequestOpts{JSON: body})
					if err != nil {
						return err
					}
					return output.JSONRaw(raw)
